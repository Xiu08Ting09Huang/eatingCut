  ...
  <div class="userlist-container">
    <div>
      ...
      <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="商户图片" prop="pic" ref="uploadpic" >
          <el-upload
            class="avatar-uploader"
            action="https://tango.heeyhome.com/cx/web/upload/uploadFile"
            :show-file-list="false"
...
export default {
  data() {
    ...
    var checkSalesId = (rule, value, callback) => {
      if (value==='') {
        return callback(new Error('推荐员ID不能为空'))
      }
      setTimeout(() => {
      if (isNaN(value)) {
          callback(new Error('请输入数字值'))
    ...
    return{
      ...
      rules2: {
        ...
        pic: [ { required:true ,message: '商户图片不能为空', trigger: 'input' }],
        ...
        license: [{required:true ,message: '商户证书不能为空', trigger: 'input' }],
